 ...
 * Ordo Database
 * 
 * SQLite-backed persistent state for agents.
...
export function createDatabase(dbPath: string): OrdoDatabase {
  ...
  const insertInferenceCost = (record: InferenceCostRecord): void => {
    db.prepare(
      `INSERT INTO inference_costs (id, agent_id, model, prompt_tokens, completion_tokens, total_tokens, cost_cents, latency_ms, turn_id, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      record.id,
      record.agentId,
  ...
  const getInferenceCosts = (
    agentId?: string,
    limit?: number,
  ): InferenceCostRecord[] => {
    const query = agentId
      ? "SELECT * FROM inference_costs WHERE agent_id = ? ORDER BY created_at DESC" +
        (limit ? " LIMIT ?" : "")
      : "SELECT * FROM inference_costs ORDER BY created_at DESC" +
        (limit ? " LIMIT ?" : "");
    ...
    const rows = db.prepare(query).all(...params) as any[];
    return rows.map(deserializeInferenceCost);
  };

  const getInferenceCostsByModel = (
    model: string,
    agentId?: string,
  ): InferenceCostRecord[] => {
    const query = agentId
      ? "SELECT * FROM inference_costs WHERE model = ? AND agent_id = ? ORDER BY created_at DESC"
      : "SELECT * FROM inference_costs WHERE model = ? ORDER BY created_at DESC";

    const params = agentId ? [model, agentId] : [model];
    const rows = db.prepare(query).all(...params) as any[];
    return rows.map(deserializeInferenceCost);
  };

  const getTotalInferenceCost = (agentId?: string): number => {
    const query = agentId
      ? "SELECT SUM(cost_cents) as total FROM inference_costs WHERE agent_id = ?"
      : "SELECT SUM(cost_cents) as total FROM inference_costs";
...
function deserializeInferenceCost(row: any): InferenceCostRecord {
  return {
    id: row.id,
    agentId: row.agent_id,
    model: row.model,
