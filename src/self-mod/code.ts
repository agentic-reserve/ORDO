 ...
 *
 * Allows agents to edit their own code and configuration.
 * All changes are audited, rate-limited, and some paths are protected.
 *
 * Safety model inspired by trust boundary architecture:
...
const PROTECTED_FILES: readonly string[] = Object.freeze([
  ...
  "constitution.md",
  // Defense infrastructure (agents must not modify their own guardrails)
 ...
 */
function isRateLimited(db: OrdoDatabase): boolean {
...
export async function editFile(
  ...
  let oldContent = "";
  try {
    oldContent = await client.readFile(filePath);
  } catch {
    oldContent = "(new file)";
  }
  ...
  // 6. Pre-modification git snapshot
  try {
    await client.gitCommit(`pre-modify: ${reason}`, "snapshot");
  } catch {
  ...
  try {
    await client.gitCommit(reason, "self-mod");
  } catch {
